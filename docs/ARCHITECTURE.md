...
### 11.1. Хардкоды идентификаторов ресторана:
```text
Основной склад (приход): 1239d270-1bbe-f64f-b7ea-5f00518ef508
iiko Store ID:           170243
Отделение (Department):  "The Lokmaсo" (кириллическая 'с')
Посадочные места:        51 (Зал: 39, VIP: 3, Улица: 9)
Бакет Supabase Storage:  "invoices" (для фото накладных и товаров)
```

---

### 11.2. Справочник единиц измерения iiko (GUID → Ед):
| GUID в iiko | Единица измерения |
| :--- | :---: |
...
### 11.3. Матрица прав доступа (`hasAccess`):
| Раздел | ID вкладки | Допустимые роли |
| :--- | :--- | :--- |
| **Опись основных средств (ОС)** | `fixed_assets` | `admin`, `manager` *(закрыто для всех остальных)* |
| **Отчеты (Месячная касса)** | `reports` | **ТОЛЬКО `admin`** |
| **Сотрудники** | `employees` | **ТОЛЬКО `admin`** |
| **Инвентаризация (сличительная)** | `inventory` | **ТОЛЬКО `admin`** |
| **Агент (AI-аналитик)** | `agent` | **ТОЛЬКО `admin`** |
| **Налоговый отчет** | `tax_report` | `director`, `admin` |
| **Аналитика / P&L** | `analytics` | `director`, `manager`, `admin` |
| **Услуги** | `services` | `supplier`, `director`, `admin` |
| **Приход товаров** | `incoming` | `supplier`, `admin` |
| **Перемещение** | `transfer` | `kitchen`, `prep_chef`, `bar`, `supplier`, `hall`, `admin` |
| **Приготовление (ПФ)** | `production` | `prep_chef`, `bar`, `admin` |
| **Касса** | `cash` | `cashier`, `admin` |
| **Остатки склада** | `balances` | Все роли кроме `manager` |
| **Списание продуктов** | `writeoff` | ❌ **Заблокировано для всех** (включая `admin`), HTTP 403 |

---

### 11.4. Фильтрация номенклатуры приходов
* В приходах должны отображаться **исключительно сырьевые товары (`type === "GOODS"`)**.
* Полуфабрикаты кухни (`PREPARED`, `ПФ`) должны быть полностью отфильтрованы по типу и регулярному выражению:
  ```javascript
  /^(пф\s|пф\.|пф-|п\/ф|\[пф\]|\(пф\)|полуфабрикат)/i
  ```

---

### 11.5. Формула сверки кассы (НЕ МЕНЯТЬ)
...
### 11.6. Ограничения на запись и безопасность
1. **Только чтение в iiko**: Запрещено вызывать методы удаления/изменения техкарт или справочников iiko через API.
2. **Аудит правок смен**: Любой `PATCH` в `/api/iiko/cash` обязан сохранять историю изменений в поле `details.edit_history` и создавать запись в `bot_actions` (`action_type: "cash_edit"`).
3. **Валидация ролей на сервере**: Любое действие администратора или менеджера должно повторно валидироваться на бэкенде в `app/api/iiko/*` через `lib/auth.js`, а не только скрытием кнопок в UI.
