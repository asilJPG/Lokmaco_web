 ...
 *
 * Принтер настроен слать с адреса самого ящика, поэтому здесь он же. Побочный
 * эффект приятный: письмо с фотографией, отправленное себе с телефона, тоже
 * станет сканом — запасной путь, если принтер недоступен.
 */
var ALLOWED_SENDERS = ['qweeqw489@gmail.com'];
...
function forwardScans() {
  ...
  // ⚠️ Никакого `is:unread` в запросе. Принтер шлёт скан с адреса самого
  // ящика, а письмо себе Gmail помечает прочитанным сразу — по `is:unread`
  // скрипт не нашёл бы ни одного письма и молча ничего не делал.
  //
  // От повторной отправки защищает ярлык, а не признак прочтения. Окно в двое
  // суток — чтобы после долгого простоя скрипт не начал разбирать всю почту.
