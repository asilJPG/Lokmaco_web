...
export async function sendInvoiceReport(payload) {
  try {
    ...
    if (files.length === 1) {
      ...
      if (!res.ok) console.error(`[telegram] sendPhoto: бот ${BOT_ID} → чат ${TG_CHAT_ID}:`, res.status, await res.text());
      return res.ok;
    }
    ...
    if (!res.ok) console.error(`[telegram] sendMediaGroup: бот ${BOT_ID} → чат ${TG_CHAT_ID}:`, res.status, await res.text());
    return res.ok;
  } catch (e) {
