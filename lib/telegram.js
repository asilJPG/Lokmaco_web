...
async function sendText(text) {
  ...
  if (!res.ok) console.error(`[telegram] sendMessage в чат ${TG_CHAT_ID}:`, res.status, await res.text());
  return res.ok;
}
...
export async function sendInvoiceReport(payload) {
  try {
    ...
    if (files.length === 1) {
      ...
      if (!res.ok) console.error(`[telegram] sendPhoto в чат ${TG_CHAT_ID}:`, res.status, await res.text());
      return res.ok;
    }
    ...
    if (!res.ok) console.error(`[telegram] sendMediaGroup в чат ${TG_CHAT_ID}:`, res.status, await res.text());
    return res.ok;
  } catch (e) {
