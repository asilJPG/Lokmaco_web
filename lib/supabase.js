...
export async function deleteTag(code) {
  try {
    const res = await fetch(
      `${SUPABASE_URL}/rest/v1/asset_tags?code=eq.${encodeURIComponent(code)}&asset_id=is.null`,
      { method: "DELETE", headers: getHeaders() }
    );
    return res.ok;
  } catch (e) {
    console.error("[supabase] deleteTag:", e.message);
    return false;
  }
}
...
// Обеды руководства
//
// Учёт живёт только на сайте: в iiko такие заказы закрываются «без оплаты»,
// чтобы не садиться на выручку. Отсюда — никаких запросов к iiko.
// ---------------------------------------------------------------------------

export async function getManagerLimits() {
  try {
    const url = `${SUPABASE_URL}/rest/v1/manager_limits?select=*&order=manager_name.asc`;
    const res = await fetch(url, { headers: getHeaders() });
    if (!res.ok) {
      console.error("[supabase] getManagerLimits:", res.status, await res.text());
      return [];
    }
    return await res.json();
  } catch (e) {
    console.error("[supabase] getManagerLimits:", e.message);
    return [];
  }
}

/** Записи за период. Границы включительные — их считает вызывающий. */
export async function getManagerMeals(dateFrom, dateTo) {
  try {
    const url =
      `${SUPABASE_URL}/rest/v1/manager_meals?select=*` +
      `&date=gte.${dateFrom}&date=lte.${dateTo}` +
      `&order=date.desc,created_at.desc`;
    const res = await fetch(url, { headers: getHeaders() });
    if (!res.ok) {
      console.error("[supabase] getManagerMeals:", res.status, await res.text());
      return [];
    }
    return await res.json();
  } catch (e) {
    console.error("[supabase] getManagerMeals:", e.message);
    return [];
  }
}

export async function createManagerMeal(row) {
  try {
    const res = await fetch(`${SUPABASE_URL}/rest/v1/manager_meals`, {
      method: "POST",
      headers: { ...getHeaders(), Prefer: "return=representation" },
      body: JSON.stringify(row),
    });
    if (!res.ok) {
      console.error("[supabase] createManagerMeal:", res.status, await res.text());
      return { error: "Не удалось сохранить запись" };
    }
    const rows = await res.json();
    return { row: rows[0] };
  } catch (e) {
    console.error("[supabase] createManagerMeal:", e.message);
    return { error: "Не удалось сохранить запись" };
  }
}

export async function getManagerMealById(id) {
  try {
    const url = `${SUPABASE_URL}/rest/v1/manager_meals?id=eq.${id}&select=*&limit=1`;
    const res = await fetch(url, { headers: getHeaders() });
    if (!res.ok) return null;
    const rows = await res.json();
    return rows[0] || null;
  } catch (e) {
    console.error("[supabase] getManagerMealById:", e.message);
    return null;
  }
}

export async function deleteManagerMeal(id) {
  try {
    const res = await fetch(`${SUPABASE_URL}/rest/v1/manager_meals?id=eq.${id}`, {
      method: "DELETE",
      headers: getHeaders(),
    });
    return res.ok;
  } catch (e) {
    console.error("[supabase] deleteManagerMeal:", e.message);
    return false;
  }
}

/** Создать или обновить лимит руководителя. Имя — первичный ключ. */
export async function upsertManagerLimit(managerName, monthlyLimit) {
  try {
    const res = await fetch(
      `${SUPABASE_URL}/rest/v1/manager_limits?on_conflict=manager_name`,
      {
        method: "POST",
        headers: {
          ...getHeaders(),
    ...
    if (!res.ok) {
      console.error("[supabase] upsertManagerLimit:", res.status, await res.text());
      return { error: "Не удалось сохранить лимит" };
    }
    const rows = await res.json();
    return { row: rows[0] };
  } catch (e) {
    console.error("[supabase] upsertManagerLimit:", e.message);
    return { error: "Не удалось сохранить лимит" };
  }
}

export async function deleteManagerLimit(managerName) {
  try {
    const res = await fetch(
      `${SUPABASE_URL}/rest/v1/manager_limits?manager_name=eq.${encodeURIComponent(managerName)}`,
      { method: "DELETE", headers: getHeaders() }
    );
    return res.ok;
  } catch (e) {
    console.error("[supabase] deleteManagerLimit:", e.message);
    return false;
  }
}

/** Руководитель по токену из QR. Токен — uuid, подобрать нельзя. */
export async function getManagerByToken(token) {
  try {
    const url =
      `${SUPABASE_URL}/rest/v1/manager_limits?qr_token=eq.${encodeURIComponent(token)}` +
      `&select=*&limit=1`;
    const res = await fetch(url, { headers: getHeaders() });
    if (!res.ok) return null;
    const rows = await res.json();
    ...
    console.error("[supabase] getManagerByToken:", e.message);
    return null;
  }
}
