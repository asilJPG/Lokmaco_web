...
// Hardcoded from bot.py (conception, containerId, store)
const CONCEPTION_ID = "2609b25f-2180-bf98-5c1c-967664eea837";
const CONTAINER_ID = "7ba81c3a-8de5-8f9d-fb9f-e39efcbc57cc";
...
export async function createTransfer(storeFrom, storeTo, items, comment = "") {
  ...
  // In Node 18+ fetch, cookies are handled via headers manually
  const now = new Date();
  ...
  for (const url of loginUrls) {
    try {
      ...
      if (res.status === 200 && body.replace(/\s/g, "").includes('"authorized":true')) {
  ...
  if (!loggedIn) {
    return { success: false, error: "iikoWeb auth failed" };
  }
  ...
  try {
    ...
    const createRes = await fetch(`${IIKO_WEB_URL}/api/documents/create`, {
      method: "POST",
      headers,
    ...
    await fetch(`${IIKO_WEB_URL}/api/documents/get/${docId}?type=INTERNAL_TRANSFER`, { headers });

    // ─── Step 4: Save with items as PROCESSED ───
    ...
    const saveRes = await fetch(`${IIKO_WEB_URL}/api/documents/save/${docId}`, {
      method: "POST",
      headers,
    ...
    if (saveRes.status === 200 && !saveData.error) {
      console.log(`[iikoWeb] ✅ Transfer ${docNumber} processed`);
