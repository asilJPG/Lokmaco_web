...
export async function createWriteoff(storeId, items, comment = "") {
  return await withIikoSession(async (token) => {
    const body = {
      dateIncoming: new Date().toISOString().substring(0, 16),
      status: "PROCESSED",
      storeId: storeId,
      accountId: "6f983109-eb1f-4517-917b-9912d5eeda16", // Пищевые потери и списания
      items: items.map((it) => ({
