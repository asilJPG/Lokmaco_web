/**
 * Определения инструментов агента и диспетчер.
 *
 ...
 */
import type { IikoCreds } from '@/lib/iiko';
import {
  avgCheckByPlace,
  guestsPerDay,
  turnover,
...
export const TOOLS = [
  {
    ...
    input_schema: { type: 'object' as const, properties: { ...dateRange }, required: ['from', 'to'] },
  },
  {
    name: 'guests_per_day',
    description:
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: { ...dateRange, limit: { type: 'integer', description: 'Сколько позиций в каждом рейтинге, по умолчанию 20' } },
      required: ['from', 'to'],
    },
  },
  {
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...dateRange,
        account_name: { type: 'string', description: 'Точное название счёта как в iiko, например «Налог» или «Аренда помещение»' },
      },
...
type Handler = (input: any, creds: IikoCreds) => Promise<unknown>;
...
/** Человекочитаемая подпись для индикатора в UI. */
...
export async function runTool(name: string, input: unknown, creds: IikoCreds): Promise<unknown> {
  const handler = HANDLERS[name];
  if (!handler) return { error: true, message: `Неизвестный инструмент: ${name}` };
  try {
    return await handler(input || {}, creds);
  } catch (e) {
